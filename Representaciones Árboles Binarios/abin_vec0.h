...
template <typename T>
int profundidad_nodo(typename Abin<T>::nodo n) const
{
    assert(n < numNodos);
...
int altura_nodo(typename Abin<T>::nodo n) const
{
    if(n == NODO_NULO)
        return -1;
    else
        return 1 + max(altura_nodo(hijoIzqdoB(n)), altura_nodo(hijoDrchoB(n)));
}
