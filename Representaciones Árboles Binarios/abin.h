...
template <typename T>
int Abin<T>::profundidad_nodo(typename Abin<T>::nodo n) const
{
    if(n == Abin<T>::NODO_NULO)
        return -1;
    else
        return 1 + profundidad_nodo(padreB(n));
}

template <typename T>
int Abin<T>::altura_nodo(typename Abin<T>::nodo n) const
{
    if(n == Abin<T>::NODO_NULO)
        return -1;
    else
        return 1 + std::max(altura_nodo(hijoIzqdoB(n)), altura_nodo(hijoDrchoB(n)));
}
